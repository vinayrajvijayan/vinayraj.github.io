...
echo "ooooo      TOMCAT INSTALL      ooooo" >> /home/$user/install.progress.txt

echo "Initial Tomcat setup" >> /home/$user/install.progress.txt
...
cd /tmp
curl -O http://mirror.fibergrid.in/apache/tomcat/tomcat-8/v8.0.44/bin/apache-tomcat-8.0.44.tar.gz
sudo mkdir /etc/tomcat
sudo tar xzvf apache-tomcat-8*tar.gz -C /etc/tomcat --strip-components=1
cd /etc/tomcat
...
sudo echo "Environment=JAVA_HOME=/usr/lib/jvm/java-1.8.0-openjdk-amd64/jre" > /etc/systemd/system/tomcat.service
sudo echo "Environment=CATALINA_PID=/etc/tomcat/temp/tomcat.pid" > /etc/systemd/system/tomcat.service
sudo echo "Environment=CATALINA_HOME=/etc/tomcat" > /etc/systemd/system/tomcat.service
sudo echo "Environment=CATALINA_BASE=/etc/tomcat" > /etc/systemd/system/tomcat.service
...
sudo echo "ExecStart=/etc/tomcat/bin/startup.sh" > /etc/systemd/system/tomcat.service
sudo echo "ExecStop=/etc/tomcat/bin/shutdown.sh" > /etc/systemd/system/tomcat.service
...
sudo cp /etc/tomcat/conf/server.xml /etc/tomcat/conf/ORIG_server.xml
sudo sed -i '/Connector on port 8009 -->/{n;d}' /etc/tomcat/conf/server.xml
sudo sed -i '/protocol=\"AJP\/1.3\" redirectPort=\"8443\"/{n;d}' /etc/tomcat/conf/server.xml
...
echo "Done." >> /home/$user/install.progress.txt
sudo /bin/date +%H:%M:%S >> /home/$user/install.progress.txt


echo "Configuring Tomcat manager" >> /home/$user/install.progress.txt

# Setup permissions for the Tomcat manager
sudo mv /etc/tomcat/conf/tomcat-users.xml /etc/tomcat/conf/ORIG_tomcat-users.xml
...
sudo echo "</tomcat-users>" >> /tmp/tomcat-users.xml
sudo mv /tmp/tomcat-users.xml /etc/tomcat/conf/
sudo chown root.tomcat /etc/tomcat/conf/tomcat-users.xml 
sudo chmod 0640 /etc/tomcat/conf/tomcat-users.xml
...
echo "Configuring SSH" >> /home/$user/install.progress.txt

# Change group of user to same as Tomcat 8
sudo usermod -g tomcat $user
...
ssh-keygen -q -N $passphrase -f /home/$user/.ssh/id_rsa
cd /home/$user/.ssh
sudo cp id_rsa.pub authorized_keys
chown -R $user.tomcat .
chown -R $user.tomcat *
echo "SSH User name:  "$user > /home/$user/vsts_ssh_info
echo "SSH passphrase: "$passphrase >> /home/$user/vsts_ssh_info
echo "SSH Private key:" >> /home/$user/vsts_ssh_info
cat id_rsa >> /home/$user/vsts_ssh_info
chown $user.tomcat /home/$user/vsts_ssh_info

echo "Done." >> /home/$user/install.progress.txt
sudo /bin/date +%H:%M:%S >> /home/$user/install.progress.txt


echo "Installing and Configuring FTP" >> /home/$user/install.progress.txt
...
echo "These original files were saved in case you want to return to default settings:" >> /home/$user/install.progress.txt
sudo find /etc -name ORIG_* -print >> /home/$user/install.progress.txt
